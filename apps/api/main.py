...
app = FastAPI(title="AEGIS API", version="0.2.4", description="Autonomous Enterprise Graph Intelligence System")

app.add_middleware(
    CORSMiddleware,
@ -25,288 +20,170 @@ app.add_middleware(
    allow_headers=["*"],
)
...
async def root():
    ...
    return {"status": "online", "message": "AEGIS API is running", "docs": "/docs", "ui": "/ui", "version": "0.2.4", "graph_loaded": graph is not None, "graph_error": graph_load_error}
        ...
        try:
            async for event in graph.astream_events({"task": req.input, "messages": [HumanMessage(content=req.input)]}, config=config, version="v2"):
                ...
                if kind == "on_chat_model_stream":
                    ...
                    # Fallback for nodes that use .invoke() instead of .astream()
                    if run_id not in streamed_runs:
                        output = event["data"].get("output")
                        if output:
                            token = output.content if hasattr(output, "content") else str(output)
                            if token and not token.startswith("function"):
                                yield f"data: {json.dumps({'token': token})}\n\n"
        except Exception as e:
